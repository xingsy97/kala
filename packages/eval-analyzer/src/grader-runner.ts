...
export class EvaluationGrader {
  ...
  private async execute(queued: AnalysisJob, signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      if (!controller.signal.aborted) await this.options.controlPlane.command(command('analysis.job.fail', { jobId: queued.jobId, executorId: this.options.executorId, failure: { code: 'GRADING_EXECUTION_FAILED', summary: safeMessage(error) } })).catch(() => undefined)
    } finally {
  ...
  private async upload(job: AnalysisJob, result: GradingResult, signal: AbortSignal) {
    ...
    await this.options.controlPlane.stageAnalysisArtifact({ jobId: job.jobId, executorId: this.options.executorId, path: artifactRef, mediaType: 'application/json', bytes: content.byteLength, sha256, content }, signal)

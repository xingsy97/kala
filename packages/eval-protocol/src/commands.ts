...
export const EvaluationCommandSchema = z.discriminatedUnion('type', [
  ...
  CommandEnvelopeSchema.extend({ type: z.literal('analysis.job.heartbeat'), jobId: IdentifierSchema, executorId: IdentifierSchema, leaseMs: z.number().int().positive(), leaseToken: NonEmptyStringSchema, generation: z.number().int().positive() }),
  CommandEnvelopeSchema.extend({ type: z.literal('analysis.job.complete'), jobId: IdentifierSchema, executorId: IdentifierSchema, leaseToken: NonEmptyStringSchema, generation: z.number().int().positive(), outputManifest: AnalysisOutputManifestSchema }),
  CommandEnvelopeSchema.extend({ type: z.literal('analysis.job.fail'), jobId: IdentifierSchema, executorId: IdentifierSchema, leaseToken: NonEmptyStringSchema, generation: z.number().int().positive(), failure: z.object({ code: IdentifierSchema, summary: NonEmptyStringSchema }).strict() }),

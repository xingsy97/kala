...
const logger = createRuntimeLogger('programbench-legacy-runner')
...
async function main(): Promise<void> {
  ...
  const root = join(args.legacy-runnerDir, 'artifacts', 'program-bench')
  ...
  for (const item of cases) {
    ...
    for (const agent of args.agents) {
      ...
      progress.current = { agent, instance_id: item.instance_id, case_root: relativeToLegacy Runner(args.legacy-runnerDir, caseRoot) }
      ...
      await writeFile(manifestPath, `${JSON.stringify({
        ...
        instance: item,
        workspace_root: relativeToLegacy Runner(args.legacy-runnerDir, workspaceRoot),
        prompt: relativeToLegacy Runner(args.legacy-runnerDir, promptPath),
        case_json: relativeToLegacy Runner(args.legacy-runnerDir, caseJsonPath),
        response: relativeToLegacy Runner(args.legacy-runnerDir, responsePath),
        submission_archive: relativeToLegacy Runner(args.legacy-runnerDir, join(submissionDir, 'submission.tar.gz')),
        eval_json: relativeToLegacy Runner(args.legacy-runnerDir, join(submissionDir, `${item.instance_id}.eval.json`)),
        native_eval_command: `programbench eval ${relativeToLegacy Runner(args.legacy-runnerDir, join(runRoot, 'submissions', agent))} --filter '^${escapeRegex(item.instance_id)}$' --workers 1`,
        ...
        bash_isolation: agent === 'agent-runlab'
          ...
          : {
              ...
              image: item.image_name ? `${item.image_name}:task_cleanroom_v6` : null,
              workspace_scope: relativeToLegacy Runner(args.legacy-runnerDir, workspaceRoot),
            },
        contract_repair: {
      ...
      if (!args.dryRun && shouldSkipProgramBenchAgentRun(stopReasonBeforeCase)) {
        ...
        try {
          ...
          const completionControl = classifyProgramBenchCompletionControl({
            contract,
            responseText: await latestProgramBenchResponseText(args.legacy-runnerDir, caseRoot, responsePath),
          })
      ...
      const artifactRefs = [
        ...
        relativeToLegacy Runner(args.legacy-runnerDir, caseJsonPath),
        relativeToLegacy Runner(args.legacy-runnerDir, manifestPath),
        relativeToLegacy Runner(args.legacy-runnerDir, join(submissionDir, 'submission.tar.gz')),
      ]
      if (submissionContract) artifactRefs.push(relativeToLegacy Runner(args.legacy-runnerDir, contractPath))
      for (const extra of ['skipped.txt', 'agent-error.txt', 'controlled-stop.txt', 'error.txt', 'isolation-warning.txt', 'native-score.json', 'native-eval-timeout.json', 'compile-probe.json', 'compile-repair-gate.json', 'compile-repair-prompt.txt', 'compile-repair-summary.json', 'submission-contract.initial.json', 'submission-contract.json', 'submission-contract-gate.json', 'submission-contract-normalization.initial.json', 'submission-contract-normalization.repair.json', 'contract-repair-prompt.txt', 'contract-repair-response.txt', 'contract-repair-error.txt', 'contract-continuation-summary.json']) {
        ...
        if (existsSync(extraPath)) artifactRefs.push(relativeToLegacy Runner(args.legacy-runnerDir, extraPath))
      }
      for (const extraPath of await collectContractContinuationArtifacts(caseRoot)) {
        artifactRefs.push(relativeToLegacy Runner(args.legacy-runnerDir, extraPath))
      }
...
function printHelp(): void {
  process.stdout.write(`Usage: pnpm --filter @agent-kernel/host exec tsx bin/run-programbench-legacy-runner.ts [options]

Options:
  --legacy-runner-dir <path>              Legacy Runner root (default: experiments/evals/2026-07-agent-benchmark-comparison)
...
async function runContractContinuationLoop(options: ContractContinuationLoopOptions): Promise<{
  ...
  for (let attempt = 1; attempt <= maxAttempts && !contract.ok; attempt++) {
    ...
    attempts.push({
      attempt,
      prompt: relativeToLegacy Runner(options.args.legacy-runnerDir, promptPath),
      response: relativeToLegacy Runner(options.args.legacy-runnerDir, responsePath),
...
async function programBenchSourceFingerprint(workspaceRoot: string): Promise<string> {
  ...
  for (const file of files.sort()) {
    hash.update(relativeToLegacy Runner(workspaceRoot, file))
    hash.update('\0')
  ...
  try {
    ...
    if (err instanceof CommandTimeoutError) {
      ...
      await writeFile(join(caseRoot, 'native-eval-timeout.json'), `${JSON.stringify({
        ...
        log: relativeToLegacy Runner(args.legacy-runnerDir, err.logPath),
...
async function latestProgramBenchResponseText(legacy-runnerDir: string, caseRoot: string, responsePath: string): Promise<string> {
  ...
  const latestPath = latestContinuationResponse
    ? resolveProgramBenchArtifactPath(legacy-runnerDir, caseRoot, latestContinuationResponse)
    : responsePath
...
function resolveProgramBenchArtifactPath(legacy-runnerDir: string, caseRoot: string, artifactPath: string): string {
  if (artifactPath.startsWith('/')) return artifactPath
  if (artifactPath.startsWith('artifacts/')) return join(legacy-runnerDir, artifactPath)
...
async function writeOutputs(
  ...
  const gradingDir = join(args.legacy-runnerDir, 'artifacts', 'program-bench', 'grading')
  ...
  const summary = {
    ...
    agents: Object.fromEntries(args.agents.map((agent) => {
      ...
      return [agent, {
        ...
        compile_repair_attempts: rows.reduce((sum, row) => sum + compileRepairAttemptCount(args.legacy-runnerDir, row.artifact_refs), 0),
      }]
    })),
  ...
  if (updateHeadline) {
    ...
    await mkdir(join(args.legacy-runnerDir, 'reports', 'programbench'), { recursive: true })
    await writeFile(join(args.legacy-runnerDir, 'reports', 'programbench', 'latest-run-summary.json'), `${JSON.stringify(summary, null, 2)}\n`, 'utf8')
  }
...
async function writeProgramBenchDiagnostic(
  ...
  const diagnosticsDir = join(args.legacy-runnerDir, 'diagnostics', 'programbench')
  await mkdir(diagnosticsDir, { recursive: true })
  const modelConfigErrors = results.filter((row) => row.error_type?.includes('model_not_found'))
  const report = {
    schema_version: 1,
    generated_by: 'packages/host/bin/run-programbench-legacy-runner.ts',
    benchmark: 'program-bench',
    run_id: args.runId,
...
async function writePairwise(args: Args, runRoot: string, results: readonly InstanceResult[]): Promise<void> {
  ...
  if (!args.dryRun && shouldUpdateProgramBenchHeadline(results)) {
    const reportRoot = join(args.legacy-runnerDir, 'reports', 'programbench')
...
function compileRepairAttemptCount(legacy-runnerDir: string, artifactRefs: readonly string[]): number {
  void legacy-runnerDir
...
function parseArgs(argv: readonly string[]): Args {
  ...
  const casesPath = resolve(value(argv, '--cases') ?? join(legacy-runnerDir, 'planning/programbench/pilot-cases.jsonl'))

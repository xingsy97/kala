...
const logger = createRuntimeLogger('jobbench-portfolio')
const REPO_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '../../..')

async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2))
  const root = join(args.portfolioDir, 'artifacts', 'job-bench')
  ...
  for (const item of selectedCases) {
    for (const agent of args.agents) {
      ...
      const artifactRefs = [
        ...
        relativeToPortfolio(args.portfolioDir, responsePath),
        relativeToPortfolio(args.portfolioDir, rubricPath),
        relativeToPortfolio(args.portfolioDir, taskCardPath),
        relativeToPortfolio(args.portfolioDir, refManifestPath),
        relativeToPortfolio(args.portfolioDir, deliverablesManifestPath),
      ]
      ...
      if (!args.dryRun) {
        ...
        await writeFile(deliverablesManifestPath, `${JSON.stringify({ schema_version: 1, workspace_root: relativeToPortfolio(args.portfolioDir, workspaceRoot), files: [] }, null, 2)}\n`, 'utf8')
      }
...
async function writeSelectedCases(args: Args, selected: readonly JobBenchCase[]): Promise<void> {
  ...
  await writeFile(join(args.portfolioDir, 'reports/jobbench/benchmark-summary.json'), `${JSON.stringify(summary, null, 2)}\n`, 'utf8')
}
...
async function writeOutputs(
  ...
  await mkdir(join(args.portfolioDir, 'reports', 'jobbench'), { recursive: true })
  await writeFile(join(args.portfolioDir, 'reports', 'jobbench', 'latest-run-summary.json'), `${JSON.stringify(summary, null, 2)}\n`, 'utf8')
...
async function writePairwise(args: Args, runRoot: string, results: readonly InstanceResult[]): Promise<void> {
  ...
  const reportRoot = join(args.portfolioDir, 'reports', 'jobbench')
...
function parseArgs(argv: readonly string[]): Args {
  ...
  const selectedCasesPath = resolve(value(argv, '--selected-cases') ?? join(portfolioDir, 'planning/jobbench/selected-cases.jsonl'))

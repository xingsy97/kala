...
async function main(): Promise<void> {
  ...
  const programRoot = join(args.legacy-runnerDir, 'artifacts', 'program-bench')
  ...
  const runRoot = join(args.legacy-runnerDir, latest.run_root)
  ...
  for (const agent of agents) {
    ...
    for (const instanceId of await readdir(agentRoot)) {
      ...
      written.push(relativeToLegacy Runner(args.legacy-runnerDir, out))
    }
  }
  const summary = {
    schema_version: 1,
...
function parseArgs(argv: readonly string[]): Args {
  return {
    legacy-runnerDir: resolve(REPO_ROOT, value(argv, '--legacy-runner-dir') ?? 'experiments/evals/2026-07-agent-benchmark-comparison'),
...
function relativeToLegacy Runner(legacy-runnerDir: string, target: string): string {
  return resolve(target).startsWith(resolve(legacy-runnerDir))
    ? resolve(target).slice(resolve(legacy-runnerDir).length).replace(/^\/+/, '').replaceAll('\\', '/')
    : target.replaceAll('\\', '/')
}

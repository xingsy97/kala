...
import { startDedicatedRuntimeUnit } from './dedicated-unit.js'
...
const llm: LLMAdapter = { name: 'dedicated-test', async call() { return { message: { role: 'assistant', content: [{ type: 'text', text: 'ok' }] } } } }
...
describe('Dedicated Runtime Unit composition', () => {
  it('routes the public origin to fixed local Unit with full capabilities', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dedicated-runtime-unit-')); roots.push(root)
    unit = await startDedicatedRuntimeUnit({
      sessionsDir: join(root, 'sessions'),
      llm,
      defaultConfig: createConfig({ systemPrompt: 'test', tools: [] }),
    })
    ingress = await startDedicatedIngress({ port: 0, unitOrigin: unit.origin })
    const response = await fetch(`http://127.0.0.1:${ingress.port}/runtime/capabilities`)
    expect(response.status).toBe(200)
    expect(await response.json()).toMatchObject({
      product: 'dedicated',
      deployment: { schemaVersion: 1, architecture: 'platform', tenancy: 'single-tenant', runtimeProfile: 'full' },
  ...
  it('atomically routes new requests to a newly active slot without restarting Ingress', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dedicated-ingress-slots-')); roots.push(root)
    ...
    await writeDedicatedRouteState(routePath, route)
    ingress = await startDedicatedIngress({ port: 0, unitOrigin: blue.origin, routeStatePath: routePath })
    const origin = `http://127.0.0.1:${ingress.port}`
    expect(await fetch(origin).then((response) => response.text())).toBe('blue')
    await writeDedicatedRouteState(routePath, { ...route, generation: 2, activeSlot: 'green', updatedAt: new Date().toISOString() })

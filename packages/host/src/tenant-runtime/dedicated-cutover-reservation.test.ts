...
describe('Dedicated cutover reservation', () => {
  it('enters drain only through private reservation and can release it', async () => {
    ...
    unit = await startDedicatedRuntimeUnit({ sessionsDir: join(root, 'sessions'), llm, defaultConfig: createConfig({ tools: [], systemPrompt: '' }) })

...
describe('independent Dedicated Dashboard release', () => {
  it('validates an exact versioned manifest and rejects traversal or future fields', () => {
    ...
    const value = { schemaVersion: 1, product: 'kala-dashboard', version: '1.2.3', builtAt: new Date().toISOString(), source: { revision: 'a'.repeat(40), snapshotSha256: 'b'.repeat(64), dirty: false }, protocol: { min: '1.0.0', max: '1.1.0' }, assetDigest: hashFiles(files), files }
  ...
  it('serves the active generation, SPA fallback, immutable chunks, and 404 for stale chunks', async () => {
    ...
    const shellHtml = await shell.text(); expect(shellHtml).toContain('r1'); expect(shellHtml).toContain('kala-dashboard-generation'); expect(shell.headers.get('x-kala-dashboard-generation')).toBe('4'); expect(shell.headers.get('cache-control')).toContain('no-cache')
  ...
  it('serves only manifest-verified hashed chunks from retained immutable releases', async () => {
    ...
    const manifest = (version: string, files: { path: string; bytes: number; sha256: string }[]) => ({ schemaVersion: 1, product: 'kala-dashboard', version, builtAt: new Date().toISOString(), source: { revision: 'a'.repeat(40), snapshotSha256: 'b'.repeat(64), dirty: false }, protocol: { min: '1.0.0', max: '1.0.0' }, assetDigest: hashFiles(files), files })
    ...
    expect(response.status).toBe(200); expect(await response.text()).toBe(oldChunk); expect(response.headers.get('x-kala-dashboard-release')).toBe('dashboard-r2'); expect(response.headers.get('x-kala-dashboard-asset-release')).toBe('dashboard-r1')

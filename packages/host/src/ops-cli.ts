...
export async function runEnhancementCli(command: EnhancementCliCommand): Promise<boolean> {
  ...
  if (command.kind === 'rl-run-rollout-smoke') {
    ...
    const rlSystemPrompt = [
      ...
      '',
      'You MUST interact with the workspace ONLY through tool calls. Tools: read_file, read_files, ls, glob, grep, write_file, replace_in_file, replace_many_in_file, apply_file_patch, bash.',
      '',
      ...
      '  1. `ls` the repo root and `read_file` or `read_files` any README/setup docs to understand the project layout.',
      '  2. `grep` for symbols named in the issue description to locate the offending code.',
      '  3. `read_file` or `read_files` the specific source files you plan to change.',
      '  4. Use `replace_in_file`, `replace_many_in_file`, `apply_file_patch`, or `write_file` to modify EXISTING source files in place. DO NOT create src/solution.py or any new top-level scaffolding — the fix belongs in the existing project files.',
      ...
      '<tool_call>',
      '{"name": "replace_in_file", "arguments": {"path": "django/contrib/auth/forms.py", "old_string": "old code", "new_string": "new code"}}',
      '</tool_call>',
    ].join('\n')

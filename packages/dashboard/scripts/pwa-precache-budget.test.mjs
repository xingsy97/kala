...
test('production PWA precache stays within the shell budget', () => {
  ...
  assert.ok(urls.length <= 64, `precache contains ${urls.length} entries`)
  assert.ok(urls.some((url) => /\.woff2?$/u.test(url)), 'emitted KaTeX fonts must be available offline')
  ...
  assert.ok(totalBytes <= 3.25 * 1024 * 1024, `precache contains ${totalBytes} uncompressed bytes`)

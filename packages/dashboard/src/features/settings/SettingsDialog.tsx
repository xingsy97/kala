...
const SECTIONS: readonly { key: SectionKey; label: string; hint: string }[] = [
  ...
  { key: 'notifications', label: 'settings.sections.notifications.label', hint: 'settings.sections.notifications.hint' },
]
    ...
    <div>
      ...
      {invitesQuery.isLoading ? (
        ...
        <div className="space-y-2" data-testid="executor-invite-list">
          {invites.map((invite) => {
            const inviteLabel = invite.label?.trim() || t('settings.executorAccess.untitled')
            return (
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0 flex-1 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <input
                        className="h-8 min-w-[12rem] max-w-full rounded-md border-0 bg-background px-2 text-sm font-medium text-foreground outline-none ring-1 ring-border/50 focus:ring-ring/50 disabled:opacity-70"
                    ...
                    <div className="grid gap-1 text-xs text-muted-foreground sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <InviteMeta label={t('settings.executorAccess.binding')} value={invite.workspaceId ? t('settings.executorAccess.bound') : t('settings.executorAccess.notBound')} />
                      <InviteMeta
                        label={t('settings.executorAccess.workspaceId')}
                        value={invite.workspaceId ?? t('settings.executorAccess.noWorkspaceYet')}
                        mono={Boolean(invite.workspaceId)}
                      />
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      {t('settings.executorAccess.inviteSummary', { label: inviteLabel, id: invite.id.slice(0, 12) })}
                    </div>
                  </div>
...
function InviteMeta({ label, value, mono = false }: { label: string; value: string; mono?: boolean }): JSX.Element {
  return (
    <div className="min-w-0">
      ...
      <div className={cn('mt-0.5 truncate text-foreground', mono && 'font-mono')}>{value}</div>
    </div>
  )
}

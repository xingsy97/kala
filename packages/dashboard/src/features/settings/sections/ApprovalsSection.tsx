...
export function ApprovalsSection(): JSX.Element {
 ...
 return <div><SectionHeader descriptionKind="notice" title={t('settings.sections.approvals.label')} subtitle={t('settings.approvals.subtitle')}/><div className="grid gap-3 sm:grid-cols-2">{modes.map(mode=><article key={mode.key} className={`rounded-xl border p-4 ${mode.tone}`}><div className="flex items-center justify-between"><h4 className="text-sm font-semibold">{mode.title}</h4>{mode.dangerous?<span className="rounded-full bg-rose-500/15 px-2 py-0.5 text-[0.625rem] font-semibold text-rose-600">{t('settings.approvals.highRisk')}</span>:null}</div><p className="mt-2 text-xs leading-5 text-muted-foreground">{mode.body}</p>{mode.dangerous?<code className="mt-3 block rounded bg-background/80 px-2 py-1.5 font-mono text-[0.6875rem]">AK_ALLOW_ALL_OK=1</code>:null}</article>)}</div></div>
}

...
describe('TaskGraphButton responsive graph view', () => {
  it('uses a vertical graph in portrait tablet layout', () => {
    ...
    try {
      ...
      expect(popover.className).toContain('max-h-[min(76dvh,42rem)]')
      expect(popover.className).not.toContain(' h-[min(76dvh,42rem)]')
      expect(popover.className).toContain('z-[70]')
      expect(popover.className).toContain('text-popover-foreground')
      expect(screen.getByTestId('task-graph-view').getAttribute('data-canvas-width')).toBeTruthy()
      expect(screen.getByTestId('task-graph-view').getAttribute('data-canvas-height')).toBeTruthy()
      const scrollContent = screen.getByTestId('task-graph-scroll-content')

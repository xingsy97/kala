...
function LogPreview({ model, fontSize }: { model: Extract<PreviewModel, { kind: 'log' }>; fontSize: number }): JSX.Element {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-[#0b0f14] text-slate-200" data-testid="session-file-log-preview">
    <div className="flex flex-wrap gap-1 border-b border-slate-700 p-2">{LOG_LEVELS.map((level) => <button key={level} type="button" onClick={() => setEnabled((current) => { const next = new Set(current); if (next.has(level)) next.delete(level); else next.add(level); return next })} className={cn('rounded border px-2 py-1 text-[0.625rem] uppercase', enabled.has(level) ? 'border-slate-500 bg-slate-700' : 'border-slate-800 text-slate-500')} aria-pressed={enabled.has(level)}>{level}</button>)}</div>

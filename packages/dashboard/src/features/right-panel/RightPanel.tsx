...
export function RightPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-card text-card-foreground" data-testid="right-panel">
      <div className="flex h-14 min-w-0 flex-none items-center gap-1 overflow-hidden border-b border-border/40 bg-muted/20 px-2 sm:h-11" role="tablist" aria-label={t('rightPanel.tools')}>
        <div className="grid min-w-0 flex-1 grid-cols-4 gap-0.5" data-testid="right-panel-tabs">
...
function TabButton({ active, onClick, icon, label, testId }: { active: boolean; onClick(): void; icon: ReactNode; label: string; testId: string }): JSX.Element {
  return (
    <button
      ...
      data-testid={testId}
      className={cn(
        'inline-flex h-11 w-full min-w-0 items-center justify-center gap-1 rounded-lg px-1 text-[11px] font-medium transition-colors sm:h-8 sm:px-1.5',
        active

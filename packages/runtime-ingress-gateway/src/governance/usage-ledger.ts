...
export class PostgresUsageLedger {
  ...
  async append(entry: UsageEntry): Promise<boolean> {
    const tokenValues = [entry.inputTokens, entry.outputTokens, entry.cacheReadTokens ?? 0, entry.cacheCreationTokens ?? 0]
    if (tokenValues.some((value) => !Number.isSafeInteger(value) || value < 0)) throw new Error('invalid usage token count')
  ...
  async currentMonth(organizationId: string): Promise<{ tokens: number; limit?: number; exhausted: boolean }> {
    ...
    const tokens = Number(result.rows[0].tokens), limit = result.rows[0].monthly_token_limit === null ? undefined : Number(result.rows[0].monthly_token_limit)
    return { tokens, ...(limit !== undefined ? { limit } : {}), exhausted: limit !== undefined && tokens >= limit }
  }

  async assertMonthlyTokenQuota(organizationId: string, requestedTokens = 0): Promise<{ tokens: number; limit?: number }> {
    if (!Number.isSafeInteger(requestedTokens) || requestedTokens < 0) throw new Error('invalid requested token count')
    const current = await this.currentMonth(organizationId)
    if (current.limit !== undefined && current.tokens + requestedTokens > current.limit) throw new Error('organization token quota exceeded')
    return { tokens: current.tokens, ...(current.limit !== undefined ? { limit: current.limit } : {}) }
  }
}

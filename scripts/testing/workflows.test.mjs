...
test('security and release workflows fail closed without automatic npm publishing', async () => {
  ...
  assert.match(security, /grype sbom:release\/sbom\.cdx\.json/u)
  ...
  assert.match(release, /default: 'linux-x64,linux-arm64,darwin-x64,darwin-arm64'/u)
  assert.match(release, /if \[ "\$NATIVE_TARGETS" != "\$REQUIRED_NATIVE_TARGETS" \]/u)
  assert.match(release, /partial four-platform RC releases are not supported/u)
...
test('bootstrap RC requires exactly four real hosted Portable installs and successful signed GHCR/asset workflows', async () => {
  const acceptance = await workflow('rc-acceptance.yml')
  for (const target of ['linux-x64', 'linux-arm64', 'darwin-x64', 'darwin-arm64']) assert.match(acceptance, new RegExp('target: ' + target, 'u'))
  ...
  assert.match(promotion, /agent-kernel-host-linux-x64 agent-kernel-host-linux-arm64/u)
  assert.match(promotion, /agent-kernel-host-darwin-x64 agent-kernel-host-darwin-arm64/u)

...
function unifiedBootstrap({ repo, tag, component }) {
  ...
  return bash([
    ...
    '  *)',
    '    cat >&2 <<EOF',
    'Unknown COMPONENT: "$COMPONENT". Set COMPONENT to one of:',
    '',
    '  host-frontend  Run the host process AND serve the dashboard bundle (one-command VM deploy). DEFAULT.',
    '  host           Run only the headless host process (no web UI). Pair with a separately deployed frontend.',
    '  frontend       Download and extract the dashboard bundle only; do not start any process.',
    '  executor       Run an executor that dials into an existing host. Requires HOST_URL.',
    '',
    'Examples:',
    '  wget -qO- ${BASE_URL}/run.sh | COMPONENT=host-frontend bash',
    '  wget -qO- ${BASE_URL}/run.sh | COMPONENT=host bash',
    '  wget -qO- ${BASE_URL}/run.sh | COMPONENT=frontend bash',
    '  wget -qO- ${BASE_URL}/run.sh | COMPONENT=executor HOST_URL=http://host-machine:3000 bash',
    'EOF',
    '    exit 1',
    '    ;;',
    'esac',
    ...
    '  if ! wget --tries=3 --timeout=30 --retry-connrefused -O "${WORK_DIR}/${name}" "$url"; then',
    '    echo "Failed to download ${name} from ${url}" >&2',
    '    echo "  Check network reachability and that the release tag \'${TAG}\' exists in ${REPO}." >&2',
    '    exit 1',
    '  fi',
    '}',
    '',
    ...
    '  if [ -z "$expected" ]; then',
    '    echo "SHA256SUMS has no checksum for $name (release may be missing this asset)" >&2',
    '    exit 1',
    '  fi',
    '  local actual',
    ...
    '    echo "No native binary published for platform \'${target:-unsupported}\' in this release." >&2',
    '    echo "  Available runtimes: set AGENT_KERNEL_RUNTIME=cjs (needs Node.js 22+) or unset it for auto." >&2',
    '    exit 1',
    '  fi',
    '  if ! has_node22; then',
    '    echo "No native binary for platform \'${target:-unsupported}\' and no Node.js 22+ available for the .cjs fallback." >&2',
    '    echo "  Install Node.js 22+, or run on a platform with a published native binary." >&2',
    '    exit 1',
    '  fi',
    '  download "$cjs"',
    '  verify_file "$cjs"',
    ...
    '  echo "" >&2',
    '  echo "==> Starting agent-kernel host (COMPONENT=$COMPONENT)" >&2',
    '  echo "    Binding on ${bind}:${port}" >&2',
    '  if [ "$bind" = "0.0.0.0" ]; then',
    '    echo "    Reachable from other machines at http://<this-host-ip>:${port}" >&2',
    '  else',
    '    echo "    Local access: http://${bind}:${port}" >&2',
    '    echo "    To expose on all interfaces, re-run with HOST=0.0.0.0" >&2',
    '  fi',
    '  if [ "$COMPONENT" = "host-frontend" ]; then',
    '    echo "    Serving frontend from: $FRONTEND_DIR" >&2',
    '  else',
    '    echo "    NOTE: this host is headless (COMPONENT=host). Deploy the frontend separately." >&2',
    '  fi',
    '  echo "" >&2',
    '}',
    '',
    ...
    '    download_and_extract_frontend',
    '    echo "" >&2',
    '    echo "==> Frontend bundle ready at: $FRONTEND_DIR" >&2',
    '    echo "    Serve it with any static server, for example:" >&2',
    '    echo "      cd $FRONTEND_DIR && python3 -m http.server 8080" >&2',
    '    echo "    Then in the browser open ?host=http://<host-ip>:3000 to point at your host." >&2',
    '    exit 0',
    '    ;;',
    '  host-frontend)',
    '    download_and_extract_frontend',
    ...
    '      echo "  Example: wget -qO- ${BASE_URL}/run.sh | COMPONENT=executor HOST_URL=http://host-machine:3000 bash" >&2',
    '      exit 1',
    '    fi',
  ...
  if (manifest.assets.includes('run.sh')) {
    if (manifest.component === 'all' || manifest.component === 'host') {
      lines.push(
        ...
        '',
        '```bash',
        `wget -qO- ${base}/run.sh | COMPONENT=host-frontend bash`,
        '```',
        '',
        'Run headless host only (dashboard deployed separately):',
        '',
        '```bash',
        `wget -qO- ${base}/run.sh | COMPONENT=host bash`,
        '```',
        '',
      )
    }
    if (manifest.component === 'all' || manifest.component === 'dashboard') {
      lines.push(
        ...
        '',
        '```bash',
        `wget -qO- ${base}/run.sh | COMPONENT=frontend bash`,
        '```',
        '',
      )
    }
    if (manifest.component === 'all' || manifest.component === 'executor') {
      lines.push(
        ...
        '',
        '```bash',
        `wget -qO- ${base}/run.sh | COMPONENT=executor HOST_URL=http://host-machine:3000 bash`,
        '```',
        '',
      )
    }

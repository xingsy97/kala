...
for (const item of entries) {
  const embeddedDashboard = item.component === 'host' && includeDashboard
    ...
    : ''
  const buildInfo = buildInfoBanner({ artifactKind: nativeOnly ? 'native' : 'cjs', dashboardMode: item.component === 'host' && includeDashboard ? 'embedded' : 'none' })
...
function buildInfoBanner({ artifactKind, dashboardMode }) {
  const info = {
    releaseTag: tag,
    gitCommit: gitCommit(),

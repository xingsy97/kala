...
try {
  ...
  copyFileSync(resolve(repository, 'kala-desktop-archive-keyring.gpg'), publicKey)
  ...
  run('dpkg', ['--remove', 'kala-desktop'])
  apt('update')
  apt('install', '-y', `kala-desktop=${provenance.version}`)
  assert.equal(run('dpkg-query', ['-W', '-f=${Version}', 'kala-desktop']), provenance.version)
  ...
  const finalArtifact = resolve(work, `kala-desktop_${finalVersion}_amd64.deb`)
  ...
  apt('update')
  apt('install', '--only-upgrade', '-y', 'kala-desktop')
  assert.equal(run('dpkg-query', ['-W', '-f=${Version}', 'kala-desktop']), finalVersion)

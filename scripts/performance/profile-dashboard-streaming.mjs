...
try{
 ...
 await page.waitForSelector('[data-testid^="turn-timing-"]:not([data-testid^="turn-timing-details-"])',{timeout:30000});const footers=await page.$$('[data-testid^="turn-timing-"]:not([data-testid^="turn-timing-details-"])');const footer=footers.at(-1);if(!footer)throw new Error('Turn timing footer missing');await footer.click();await page.waitForSelector('[data-testid^="turn-timing-details-"]');const timingBeforeRefresh=await timingEvidence(page);if(timingBeforeRefresh.toolCount!==8||timingBeforeRefresh.modelCalls<2||timingBeforeRefresh.overflow)throw new Error(`invalid Turn timing before refresh: ${JSON.stringify(timingBeforeRefresh)}`);await page.screenshot({path:join(evidenceRoot,'turn-timing-desktop.png')})
 await page.reload({waitUntil:'networkidle2'});await page.waitForSelector('[data-testid^="turn-timing-"]:not([data-testid^="turn-timing-details-"])',{timeout:30000});const timingAfterRefresh=await timingEvidence(page);if(!timingAfterRefresh.text.includes('8 Tools')||timingAfterRefresh.overflow)throw new Error(`Turn timing did not survive refresh: ${JSON.stringify(timingAfterRefresh)}`)
 const responsive={};for(const [name,width,height] of [['phone-390',390,844],['phone-430',430,932],['ipad',768,1024]]){await page.setViewport({width,height,deviceScaleFactor:1,isMobile:width<800,hasTouch:width<800});await page.reload({waitUntil:'networkidle2'});await page.waitForSelector('[data-testid^="turn-timing-"]:not([data-testid^="turn-timing-details-"])',{timeout:30000});const buttons=await page.$$('[data-testid^="turn-timing-"]:not([data-testid^="turn-timing-details-"]) button');const button=buttons.at(-1);if(button&&await button.evaluate(node=>node.getAttribute('aria-expanded')!=='true'))await button.click();responsive[name]=await timingEvidence(page);if(responsive[name].toolCount!==8||responsive[name].modelCalls!==2||responsive[name].overflow)throw new Error(`${name} Turn timing acceptance failed: ${JSON.stringify(responsive[name])}`);await page.screenshot({path:join(evidenceRoot,`turn-timing-${name}.png`)})}await page.setViewport({width:1440,height:900})
 ...
 const metrics=Object.fromEntries((await cdp.send('Performance.getMetrics')).metrics.map(m=>[m.name,m.value]));const report={generatedAt:new Date().toISOString(),artifactMetadata:await browserMetadata(browser,page,bundle,root),parameters:{tokenChunks:240,toolCalls:8,viewport:{width:1440,height:900}},turnTiming:{beforeRefresh:timingBeforeRefresh,afterRefresh:timingAfterRefresh,responsive},durationMs,frames,...pageData,metrics,providerCalls:call};writeFileSync(join(evidenceRoot,'report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({evidenceRoot,report},null,2))
...
async function timingEvidence(page){return await page.evaluate(()=>{const root=[...document.querySelectorAll('[data-testid^="turn-timing-"]:not([data-testid^="turn-timing-details-"])')].at(-1),details=[...document.querySelectorAll('[data-testid^="turn-timing-details-"]')].at(-1),rect=(details??root)?.getBoundingClientRect();const text=root?.textContent??'';return{text,toolCount:Number(/(\d+) Tools/.exec(text)?.[1]??0),modelCalls:Number(/(\d+) model calls/.exec(text)?.[1]??0),overflow:Boolean(rect&&(rect.left<0||rect.right>innerWidth||document.documentElement.scrollWidth>innerWidth))}})}

...
const root = resolve(process.env.PRIVATE_CLOUD_BRANDING_DIR ?? 'deploy/private-cloud/branding')
const pat = process.env.ZITADEL_PAT?.trim() || execFileSync('docker', ['run', '--rm', '--network', 'none', '-v', `${process.env.ZITADEL_BOOTSTRAP_VOLUME ?? 'agent-runlab-identity-zitadel-bootstrap'}:/bootstrap:ro`, 'alpine:3.22.2', 'cat', '/bootstrap/bootstrap.pat'], { encoding: 'utf8' }).trim()

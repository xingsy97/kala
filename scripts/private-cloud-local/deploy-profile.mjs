...
if(action==='apply'){
  if (stackExists(profile)) run('node',['scripts/private-cloud-local/backup-compose-stack.mjs'])
  else process.stdout.write('No existing Private Cloud stack; initial deployment does not require a backup.\n')
  run('node',['scripts/private-cloud-local/release-manifest.mjs'])

...
const legacy-runnerDir = path.join(root, 'experiments/evals/2026-07-agent-benchmark-comparison')
const programRoot = path.join(legacy-runnerDir, 'artifacts/program-bench')
...
function rel(file) {
  return path.relative(legacy-runnerDir, file).replaceAll(path.sep, '/')
}

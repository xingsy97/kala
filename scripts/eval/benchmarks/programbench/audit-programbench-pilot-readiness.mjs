...
const programDiagnosisRoot = path.join(legacy-runnerRoot, 'diagnostics/programbench')
  ...
  const report = {
    ...
    recommendation: {
      ...
      suggested_command_shape: [
        'pnpm --filter @agent-kernel/host exec tsx bin/run-programbench-legacy-runner.ts',
        '  --legacy-runner-dir experiments/evals/2026-07-agent-benchmark-comparison',
...
async function readSameSessionPaidDiagnostic() {
  ...
  return {
    ...
    evidence: path.relative(legacy-runnerRoot, summaryPath),
  }
}
...
async function readContractContinuationProbe() {
  ...
  return {
    ...
    claude_code: claudeContract
      ...
      : null,
    evidence: path.relative(legacy-runnerRoot, stoppedNotePath),
  }
}
...
async function readMechanismReadiness() {
  ...
  for (const gatePath of currentRunGatePaths) {
    ...
    contractGateFiles.push({
      path: path.relative(legacy-runnerRoot, gatePath),
      exists: Boolean(gate),
      ok: gate?.ok ?? null,
  ...
  return {
    ...
    same_session_manifest: sameSessionManifest
      ? {
          path: path.relative(legacy-runnerRoot, sameSessionManifestPath),
    ...
    native_eval_timeout_manifest: nativeEvalTimeoutDryRun
      ? {
          path: path.relative(legacy-runnerRoot, nativeEvalTimeoutDryRunPath),
          timeout_ms: nativeEvalTimeoutDryRun.native_eval_timeout_ms ?? null,
        }
      : null,
    native_eval_stopped_note: await fileExists(nativeEvalStoppedNote) ? path.relative(legacy-runnerRoot, nativeEvalStoppedNote) : null,
    ...
    claude_inactivity_timeout_manifest: interruptionGuardDryRun
      ? {
          path: path.relative(legacy-runnerRoot, interruptionGuardDryRunPath),
    ...
    entrypoint_completion_manifest: entrypointCompletionDryRun
      ? {
          path: path.relative(legacy-runnerRoot, entrypointCompletionDryRunPath),
    ...
    completion_control_result_fields_manifest: resultFieldsSummary
      ? {
          score_summary: path.relative(legacy-runnerRoot, resultFieldsSummaryPath),
          pairwise: path.relative(legacy-runnerRoot, resultFieldsPairwisePath),
    ...
    completion_control_action_manifest: actionCheckAgentManifest
      ? {
          agent_runlab: path.relative(legacy-runnerRoot, actionCheckAgentManifestPath),
          claude_code: path.relative(legacy-runnerRoot, actionCheckClaudeManifestPath),
    ...
    promise_loop_guard_manifest: latestCompletionControl
      ? {
          path: path.relative(legacy-runnerRoot, latestCompletionControlPath),
    ...
    compile_repair_loop_manifest: compileRepairSummary
      ? {
          score_summary: path.relative(legacy-runnerRoot, compileRepairSummaryPath),
          agent_runlab: path.relative(legacy-runnerRoot, compileRepairAgentManifestPath),
          claude_code: path.relative(legacy-runnerRoot, compileRepairClaudeManifestPath),
    ...
    single_continuation_boundary_manifest: continuationBoundarySummary
      ? {
          score_summary: path.relative(legacy-runnerRoot, continuationBoundarySummaryPath),
          agent_runlab: path.relative(legacy-runnerRoot, continuationBoundaryAgentManifestPath),
          claude_code: path.relative(legacy-runnerRoot, continuationBoundaryClaudeManifestPath),
    ...
    agent_runlab_outer_contract_repair_manifest: outerContractRepairSummary
      ? {
          score_summary: path.relative(legacy-runnerRoot, outerContractRepairSummaryPath),
          agent_runlab: path.relative(legacy-runnerRoot, outerContractRepairAgentManifestPath),
          claude_code: path.relative(legacy-runnerRoot, outerContractRepairClaudeManifestPath),
    ...
    submission_skeleton_bootstrap_manifest: bootstrapSummary
      ? {
          score_summary: path.relative(legacy-runnerRoot, bootstrapSummaryPath),
          agent_runlab: path.relative(legacy-runnerRoot, bootstrapAgentManifestPath),
          claude_code: path.relative(legacy-runnerRoot, bootstrapClaudeManifestPath),
...
function describeValidatedMechanismFix(mechanismReadiness) {
  ...
  if (mechanismReadiness.single_continuation_boundary) fixes.push('ProgramBench contract repair has a single continuation boundary: Agent RunLab same-session inside the prompt runner, Claude Code bounded SDK invocations in the legacy-runner runner')
...
async function readNativeEvalSummaries(runRoot, agent, instanceIds) {
  ...
  for (const instanceId of instanceIds) {
    ...
    summaries.push({
      ...
      eval_json: evalJsonPath ? path.relative(legacy-runnerRoot, evalJsonPath).replaceAll(path.sep, '/') : null,

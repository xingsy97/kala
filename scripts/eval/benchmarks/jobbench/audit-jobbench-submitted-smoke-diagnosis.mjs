...
const jobRoot = path.join(portfolioRoot, 'artifacts/job-bench')
const jobReportRoot = path.join(portfolioRoot, 'reports/jobbench')
const diagnosisDir = path.join(portfolioRoot, 'diagnostics/jobbench')
...
function buildDiagnosis({ latest, row, preflight, agent, claude }) {
  ...
  if (latest.status === 'submitted_unscored' && row.winner === 'not_comparable') {
    items.push({
      component: 'portfolio_evidence_level',
...
function rel(filePath) {
  if (!filePath) return ''
  const absolute = path.isAbsolute(filePath) ? filePath : path.join(portfolioRoot, filePath)
  return path.relative(portfolioRoot, absolute).replaceAll(path.sep, '/')
}

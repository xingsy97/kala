...
const portfolio = path.join(root, 'experiments/evals/2026-07-agent-benchmark-comparison')
...
async function main() {
  ...
  const portfolioAnalysis = await readText('portfolio-analysis.md')
  ...
  if (summary.status !== 'portfolio_in_progress') failures.push('portfolio status must be portfolio_in_progress until all benchmark subsets are complete')
  ...
  if (!String(objectiveCoverage.portfolio?.current_scope ?? '').includes('SWE-Marathon source/no-run calibration')) failures.push('objective coverage must record SWE-Marathon source/no-run calibration as in scope')
  const completedCoverage = new Set(objectiveCoverage.portfolio?.completed_benchmarks ?? [])
  ...
  for (const requiredIncomplete of ['browsecomp', 'swe-marathon', 'program-bench', 'job-bench']) {
    if (!(objectiveCoverage.portfolio?.incomplete_benchmarks ?? []).includes(requiredIncomplete)) {
  ...
  if (browseRows.length !== 1) failures.push(`expected one BrowseComp portfolio row, got ${browseRows.length}`)
  ...
  for (const [fileName, text] of [
    ['portfolio-analysis.md', portfolioAnalysis],
  ...
  for (const required of [
    ...
    if (!existsSync(path.join(portfolio, required))) failures.push(`missing required portfolio file: ${required}`)
  }
  ...
  for (const benchmark of ['swebench', 'browsecomp', 'swe-marathon', 'program-bench', 'job-bench']) {
    for (const required of ['runbook.md', 'command-log.md', 'failure-analysis.md', 'pairwise-comparison.csv', 'pairwise-comparison.jsonl']) {
      ...
      if (!existsSync(path.join(portfolio, relative))) failures.push(`missing per-benchmark report file: ${relative}`)
    }
  }

  for (const benchmark of ['swebench', 'browsecomp', 'swe-marathon', 'program-bench', 'job-bench']) {
    const relative = `reports/${reportDirByBenchmark[benchmark]}/benchmark-summary.json`
    if (!existsSync(path.join(portfolio, relative))) failures.push(`missing per-benchmark summary: ${relative}`)
  }
  ...
  console.log('benchmark portfolio invariants ok')
}
...
async function readJson(relativePath) {
  return JSON.parse(await readFile(path.join(portfolio, relativePath), 'utf8'))
}

async function readJsonl(relativePath) {
  const text = await readFile(path.join(portfolio, relativePath), 'utf8')
  return text.split('\n').filter(Boolean).map((line) => JSON.parse(line))
}

async function readText(relativePath) {
  return readFile(path.join(portfolio, relativePath), 'utf8')
}

...
async function main() {
  const summary = await readJson(path.join(portfolioRoot, 'results/benchmark-summary.json'))
  const benchmarks = []
  ...
  const report = {
    ...
    scope: 'Audits current portfolio headline/latest evidence and required portfolio files. It does not scan every historical diagnostic run.',
    status: benchmarks.every((item) => item.status === 'pass' || item.status === 'not_applicable') ? 'pass' : 'attention_required',
    portfolio_files: await auditPortfolioFiles(),
    benchmarks,
  }
...
async function auditPortfolioFiles() {
  const rows = []
  for (const relative of REQUIRED_RESULT_FILES) {
    rows.push(await fileCheck(relative, path.join(portfolioRoot, relative)))
  }
  return rows
}
...
async function auditBenchmark(benchmark) {
  ...
  const artifactRoot = path.join(portfolioRoot, 'artifacts', benchmark.benchmark)
  const reportRoot = path.join(portfolioRoot, 'reports', REPORT_DIR_BY_BENCHMARK[benchmark.benchmark] ?? benchmark.benchmark)
  ...
  if (benchmark.benchmark === 'browsecomp') {
    checks.push(await fileCheck('diagnostics/latest-partial-run-diagnosis.json', path.join(portfolioRoot, 'diagnostics/browsecomp/latest-partial-run-diagnosis.json')))
    checks.push(await fileCheck('diagnostics/latest-partial-run-diagnosis.md', path.join(portfolioRoot, 'diagnostics/browsecomp/latest-partial-run-diagnosis.md')))
  }
  if (benchmark.benchmark === 'program-bench') {
    checks.push(await fileCheck('diagnostics/latest-failure-diagnosis.json', path.join(portfolioRoot, 'diagnostics/programbench/latest-failure-diagnosis.json')))
    checks.push(await fileCheck('diagnostics/latest-failure-diagnosis.md', path.join(portfolioRoot, 'diagnostics/programbench/latest-failure-diagnosis.md')))
  }
  if (benchmark.benchmark === 'job-bench') {
    checks.push(await fileCheck('diagnostics/latest-submitted-smoke-diagnosis.json', path.join(portfolioRoot, 'diagnostics/jobbench/latest-submitted-smoke-diagnosis.json')))
    checks.push(await fileCheck('diagnostics/latest-submitted-smoke-diagnosis.md', path.join(portfolioRoot, 'diagnostics/jobbench/latest-submitted-smoke-diagnosis.md')))
  }
  const runRoot = latest?.run_root ? path.join(portfolioRoot, latest.run_root) : null
...
async function auditSweMarathon(benchmark) {
  const reportRoot = path.join(portfolioRoot, 'reports/swe-marathon')
  ...
  checks.push(await dirCheck('references/swe-marathon', path.join(portfolioRoot, 'references/swe-marathon')))
  return summarizeBenchmark({
    benchmark: benchmark.benchmark,
    evidence_level: benchmark.evidence_level,
...
async function auditRunRoot(benchmark, runRoot, latest) {
  ...
  if (benchmark === 'job-bench') {
    ...
    checks.push(await fileCheck('judge-preflight/preflight-summary.json', path.join(portfolioRoot, 'artifacts/job-bench/judge-preflight/preflight-summary.json')))
    checks.push(await fileCheck('judge-preflight/README.md', path.join(portfolioRoot, 'artifacts/job-bench/judge-preflight/README.md')))
  }
  return checks
}
...
function renderMarkdown(report) {
  ...
  for (const check of report.portfolio_files) lines.push(`| \`${check.label}\` | ${check.status} | ${check.size ?? check.detail ?? ''} |`)

...
const REQUIREMENTS = [
  ...
  ['evidence_notes', 'Portfolio-level evidence notes exist'],
]
...
async function main() {
  ...
  const report = {
    ...
    objective_source: 'user-provided benchmark portfolio goal in the active agent session',
    ...
    benchmarks: rows,
    portfolio: buildPortfolioCoverage(rows),
  }
...
function buildBenchmarkCoverage(benchmark, artifactCompleteness, extras) {
  ...
  set(checks, 'evidence_notes', true, 'portfolio evidence-notes.md exists and validator requires it')
...
function buildPortfolioCoverage(rows) {
...
async function readJson(relativePath) {
  return JSON.parse(await readFile(path.join(portfolioRoot, relativePath), 'utf8'))
}
...
function renderMarkdown(report) {
  const lines = [
    ...
    '',
    `Safe claim: ${report.portfolio.safe_claim}`,
    '',
    '## Benchmark Coverage',
    '',

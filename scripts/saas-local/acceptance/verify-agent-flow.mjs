...
const email = process.env.SAAS_TEST_ALICE_EMAIL?.trim(); const password = process.env.SAAS_TEST_ALICE_PASSWORD?.trim(); if (!email || !password) throw new Error('SAAS_TEST_ALICE_EMAIL and SAAS_TEST_ALICE_PASSWORD are required')
...
await page.screenshot({ path: '/tmp/saas-agent-flow.png', fullPage: false }); await browser.close()

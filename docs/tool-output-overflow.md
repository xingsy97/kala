...
- `memory` write/delete operations plus legacy `memory_write` / `memory_delete`  -  ack strings.
...
- Streaming tool output. Overflow spills after the tool completes; incremental streaming ( -  la `bash_output`) is already handled by the polling loop.

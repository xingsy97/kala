...
observability concerns. Record them in product Session traces and profiles, not
in kernel control flow. Do not expose token usage as monetary spend; pricing is
external operator context and is intentionally out of product scope.
...
- Do not encode provider pricing in kernel, shared protocol types, dashboard
  labels, or product Session summaries.
...
- Host integration and Dashboard component tests verify profile generation and
  rendering from product Session logs.
...
- Product Session profiles expose latency and token distributions with explicit
  missing-data reasons. Standalone evaluation owns cross-run aggregation and
  gates through its canonical protocol.

### Next Implementation Steps

1. Add live profile summary to the session status panel using existing usage and
   trace metadata.
2. Capture executor queue/execution timing in tool result metadata or artifacts.
3. Remove stale monetary-cost fields from profile artifact schemas in a focused
   data-model cleanup.

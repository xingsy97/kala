...
- The host derives context pressure snapshots and runs manual, hard-tier auto,
  and preflight reserve compaction.
- The host picks a recent user-message pivot and preserves that tail verbatim.
- The reducer applies a deterministic `messages_replaced` event generated by
  the host, keeping replay/fork stable without owning compaction policy.

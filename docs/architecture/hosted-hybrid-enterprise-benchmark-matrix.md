# Hosted/Hybrid Enterprise SaaS Benchmark Matrix
...
| Elastic managed runners | GitLab/GitHub offer hosted runners and autoscaling | Exclude for current positioning | No public untrusted execution pool; customers bring trusted Executors |
| Public checkout and payment | Consumer/public SaaS concern | Exclude | Manual contracts and internal entitlements only |
...
4. **Core Agent semantics remain deployment-mode neutral.** Hosted identity/routing must wrap, not fork, Session/Queue/Compact/Tool behavior.

...
| Abuse and noisy-neighbor protection | Partial. Unit resource governor supports per-unit concurrency, queue, and artifact-byte accounting. RuntimeIngressGateway has an optional tenant/principal sliding-window rate limiter with 429/retry-after behavior and tests proving limited requests do not reach the runtime upstream. Foreground shell, background shell, file reads, and live terminal streaming now have bounded output paths; terminal sessions emit a truncation marker and are terminated once their live-output budget is exceeded. POSIX foreground/background shell commands support opt-in CPU, virtual-memory, file-size, and process-count `ulimit` controls from Executor environment variables. | Container/cgroup-level executor resource controls, model spend alerts, anomaly alerts, and remaining abuse tests. |

# Hosted/Hybrid Enterprise Program Baseline

Status: frozen for implementation

## Product boundary

Agent RunLab targets trusted enterprise customers provisioned through a reviewed contract. Customers run their own outbound-only Executor fleet; Agent RunLab operates the authenticated control plane and tenant-isolated Runtime Units.
...
- Standalone release bundle: `release/bundle-dashboard-with-runtime.cjs`.
...
- Hosted Docker execution remains an acceptance environment and must not be confused with the final enterprise topology.

## Quality baseline

The preceding core-hardening program passed repository typecheck, Dashboard 763 tests, Kernel 58 tests, Executor 165 tests, Host 851 tests (2 skipped), Gateway 20 tests, release verification, desktop/mobile/PWA browser matrices, isolated Standalone smoke, and multi-tenant Runtime Unit isolation tests.

Known operational constraint: direct access to `/var/run/docker.sock` is denied for the current shell identity. Hosted deployment evidence requiring Docker must use an authorized runner rather than weakening socket permissions.

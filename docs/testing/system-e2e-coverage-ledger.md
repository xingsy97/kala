...
| GitHub Copilot Runtime | Unit/integration coverage plus `scripts/runtime/verify-agent-runtime-tools.mjs`; deployed official-provider UI canary `scripts/product-e2e/verify-copilot-runtime-journey.mjs` passed against generation 78 (`/tmp/agent-runlab-program-1788882875574-3038674-60776cd4/report.json`) | Official authentication/provider remains an external dependency, so this is retained as a required canary rather than a hermetic CI lane | Complete (deployed official-provider canary) | Production UI → Copilot Runtime selection/session ownership → actionable invalid cwd → Session Info → pasted image → approval → real Executor shell → streaming final output → Host `todo_graph` → `todowrite` → Copilot sub-agent inherited runtime → reload/recover → delete/cleanup |
...
| Restart/recovery | `scripts/product-e2e/verify-core-workspace-journeys.mjs` covers Host/browser/Executor reconnect; `scripts/product-e2e/verify-controlled-agent-journey.mjs` covers active turn plus persisted queue exactly-once; the dedicated generation 78 deploy receipt proves 87/87 planned continuation participants completed with zero failures | Executor process restart with open PTY remains a dedicated resilience extension | Complete (Host active-turn/queue/deploy handoff) | Host/Executor restart during durable operations, reconnect and exactly-once state |

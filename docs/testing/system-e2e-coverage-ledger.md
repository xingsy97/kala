...
| Direct/queued message | `scripts/product-e2e/verify-controlled-agent-journey.mjs` | Host restart while queue is pending and many-message stress remain | Complete (queue/edit/reorder/delete/reload/drain); Partial (restart stress) | Controlled protocol provider, production processes, ACK/queue/edit/reorder/delete/drain/reconnect |
| Files and preview | Core journey covers real text/binary UI; controlled-agent journey proves live hover preview | Download byte capture and explicit path-denial UI remain | Complete (read/binary/live preview); Partial (download/denial) | Real Executor list/read/write/binary/download, live preview update, reload, path denial |
...
| Sub-agent lifecycle | `scripts/product-e2e/verify-controlled-agent-journey.mjs` | Explicit provider-error branch remains; success and UI interrupt/cancel are live | Complete (success/cancel); Partial (provider failure) | Real child running/success/failure/cancel and parent transcript persistence |

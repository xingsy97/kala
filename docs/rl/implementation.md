...
The goal is not to prove that Vast can launch a GPU container or that SGLang
...
- Vast cleanup cannot be guaranteed.
...
- `vastai show instances --raw` returns `[]` after the run.
...
- actual spend reaches 25 USD without passing strict preflight on a selected
  host;
- three consecutive Vast instances fail before usable SSH;
...
- **GPU-provider authentication**: use the existing local Vast CLI

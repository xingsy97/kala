...
- ð -  **Complete**  -  all acceptance criteria met
- ð -  **In progress**
-  -  **Not started**

---

## Phase 0  -  Learning & spec (Complete) ð - 

**Purpose**: Ground the design in the actual state of the art. Avoid inventing things existing projects already handle well.

**Deliverables**:
- ð -  Clone and read the 4 reference projects: [pi](https://github.com/earendil-works/pi), [opencode](https://github.com/sst/opencode), [codex](https://github.com/openai/codex), [Claude Code source-collection](https://github.com/chauncygu/collection-claude-code-source-code)
- ð -  Produce quantitative comparison: `docs/references-comparison.md` (~2800 words, 10 kernel design lessons)
- ð -  Full design + spec: `docs/SPEC.md`, `docs/ARCHITECTURE.md`, protocol docs, ADRs
- ð -  Repository skeleton: monorepo with pnpm workspace, TypeScript strict mode, Vitest
...
## Phase 1  -  Kernel (Complete) ð - 

**Purpose**: Land the heart of the project. Everything else is scaffolding around this.

**Deliverables** (in `packages/kernel/`):
- ð -  Types: `types.ts`  -  Message / Event / State / Effect / Config
- ð -  FSM step: `core.ts`  -  pure `step(state, event, config)` (dispatch table, see [ADR 0010](adr/0010-fsm-dispatch-table.md))
- ð -  State factory: `state.ts`  -  `createInitialState()`, `createConfig()`
- ð -  Fold/fork: `fold.ts`  -  `fold`, `foldWithTrace`, `fork`
- ð -  Barrel: `index.ts`
- ð -  Tests: 23 unit tests in `core.test.ts`
...
## Phase 2  -  Host (LLM adapter + host loop + Socket.IO server) ð - 
...
## Phase 3  -  Executor (Node daemon) ð - 
...
## Phase 4  -  Dashboard (chat + inspector) ð - 
...
- Phase 2: ð -  Complete (host + LLM adapters (Anthropic + OpenAI-compatible) + JSONL log + wire protocol, 16 tests)
- Phase 3: ð -  Complete (executor + sandbox + 7 tools + CLI, 40 tests)
- Phase 4: ð -  Complete (dashboard React SPA, 7 tests)
- Phase 5: ð -  Complete (replay/fork + lineage UI + wire test)
...
- Manual compaction smoke passed against a real Host + Executor and headless Chrome: exact `/compact` emitted `client:compact`, showed visible running/done feedback, and produced `compact_replaced` JSONL entries without appending `/compact` as a user message.
